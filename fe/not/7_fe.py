...
def compute_date():
    ...
    result.append(pool.apply_async(devid_app_count, (deviceid_packages, )))
    pool.close()
    pool.join()
    ...
    deviceid_packages=deviceid_packages/deviceid_packages.std()
    deviceid_packages=pd.concat([device_id,deviceid_packages],axis=1)
